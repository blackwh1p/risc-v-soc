...
module dmem #(
    ...
    always @(posedge clk) begin
        if (write_en) begin
            if (byte_enable[0]) mem[addr[ADDR_WIDTH+1:2]] [7:0]   <= write_data[7:0];
            if (byte_enable[1]) mem[addr[ADDR_WIDTH+1:2]] [15:8]  <= write_data[15:8];
            if (byte_enable[2]) mem[addr[ADDR_WIDTH+1:2]] [23:16] <= write_data[23:16];
            if (byte_enable[3]) mem[addr[ADDR_WIDTH+1:2]] [31:24] <= write_data[31:24];
        end
    end
